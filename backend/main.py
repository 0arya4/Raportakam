...
DETECT_SYSTEM_PROMPT = """You are an advanced AI content detection system.

Your task is to analyze the given text and estimate how likely it is to be AI-generated vs human-written.

IMPORTANT RULES:
- Do NOT guess randomly. Base your judgment only on linguistic and structural evidence.
- Be skeptical of both AI and human possibilities.
- Do NOT assume all formal or well-written text is AI-generated.
- Do NOT assume all imperfect text is human-written.

ANALYZE USING THESE FACTORS:

1. Perplexity (language predictability)
   - AI text is usually highly predictable and smooth
   - Human text often has irregular phrasing

2. Burstiness (variation in sentence length and structure)
   - AI tends to have uniform sentence structure
   - Humans vary sentence lengths naturally

3. Repetition patterns
   - AI may reuse phrases or sentence structures
   - Humans are less repetitive

4. Vocabulary diversity
   - AI often uses safe, common vocabulary
   - Humans may use unusual or inconsistent word choices

5. Tone consistency
   - AI maintains consistent tone throughout
   - Humans may shift tone slightly

6. Logical flow
   - AI is often overly structured and "perfect"
   - Humans may include slight jumps or imperfections

7. Formatting patterns
   - AI frequently uses clean paragraphs and balanced structure
   - Humans may have uneven formatting
...
def build_report_prompt(req: ReportRequest) -> str:
    ...
    current_date = req.date or datetime.now().strftime("%Y-%m-%d")

    # Only include cover fields that user provided
    cover_lines = []
    if req.student_name.strip():
        cover_lines.append(f"Student: {req.student_name}")
    if req.course.strip():
        cover_lines.append(f"University: {req.course}")
    if req.instructor.strip():
        cover_lines.append(f"Instructor: {req.instructor}")
    cover_lines.append(f"Date: {current_date}")

    cover_section = "\n".join(cover_lines) if cover_lines else "No cover details provided — leave cover fields as empty strings."
...
Cover details (ONLY use these exact values, do NOT invent any):
{cover_section}
...
For cover fields not provided above, use empty string "".
...
async def report_stream(req: ReportRequest):
    ...
    max_tokens = {"Short": 4500, "Medium": 8000, "Long": 16000}.get(req.length, 8000)
    ...
    async def event_stream():
        ...
        while True:
            ...
            if kind == "stage2":
                ...
                yield f"data: {json.dumps({'url': word_url, 'final_json': full_text, 'tokens': tokens, 'done': True})}\n\n"
                break
            elif kind == "error":
