          ...
          {user ? (
            <>
                      ...
                      {history.length > 0 && (
                        <div className="border-b border-slate-800">
                          ...
                          {history.slice(0, 3).map(h => (
                            <a
                              key={h.id}
                              href={h.file_url?.startsWith('/') ? `${import.meta.env.VITE_API_URL ?? ''}${h.file_url}` : h.file_url}
                              download
              ...
              <motion.button
                ...
                className="w-8 h-8 flex items-center justify-center rounded-lg bg-slate-800 border border-slate-700 hover:border-yellow-500/40 hover:bg-yellow-500/10 transition text-slate-400 hover:text-yellow-400"
              >
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
