...
const TEMPLATES = [
  { id: 'classic',    label: 'کلاسیکی',   font: 'Times New Roman', desc: 'ئەکادیمی کلاسیک' },
  { id: 'modern',     label: 'مۆدێرن',    font: 'Calibri',          desc: 'سادە و مۆدێرن' },
  { id: 'elegant',    label: 'ئێلێگانت',  font: 'Georgia',          desc: 'ئەدیبانە و خۆش' },
  { id: 'scientific', label: 'زانستی',    font: 'Arial',            desc: 'تەکنیکی و زانستی' },
  { id: 'minimal',    label: 'مینیمال',   font: 'Calibri',          desc: 'ساکت و پاک' },
  { id: 'kurdish',    label: 'کوردی/عەرەبی', font: 'Arial',         desc: 'تایبەت بۆ RTL' },
]
          ...
          <AnimatePresence mode="wait">
            ...
            {state === 'form' && (
              <motion.div key="form" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -16 }}
                ...
                {/* Word Template */}
                <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5 space-y-3">
                  <p className="text-white font-bold text-sm">شێوازی فایلی Word</p>
                    ...
                    {TEMPLATES.map(t => (
                      <button key={t.id} onClick={() => set('template', t.id)}
                        ...
                        <div className="flex-1 min-w-0">
                          <div className={`text-xs font-black ${form.template === t.id ? 'text-yellow-400' : 'text-white'}`}>{t.label}</div>
                          <div className="text-xs text-slate-500 mt-0.5 truncate">{t.font}</div>
